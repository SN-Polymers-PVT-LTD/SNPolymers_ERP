...
/**
 * Standalone housekeeping view over the same accumulating On Hold/Rejected
 * list the import modal (ImportEligibleItemsModal, opened from within an
 * Open sheet) draws from. This page exists purely to browse and dismiss —
 * importing into a sheet still happens from within that sheet, since
 * importing always needs a target sheet_id. Dismiss here never touches the
 * source item's real data (034_add_line_item_import.sql); it only sets
 * import_dismissed so stale items stop cluttering the eligible list.
 */
    ...
    <>
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 mb-8 pb-6 border-b border-white/5">
        <div>
          ...
          <h1 className="text-3xl font-extrabold tracking-tight text-slate-100 mt-1">Held / Rejected Items</h1>
          <p className="text-xs text-slate-400 font-medium mt-1.5">
            Every On Hold or Rejected line item across all sheets that hasn't been imported into a
            new sheet yet. Import from within an Open sheet's "Import Held / Rejected" button, or
            dismiss an item here if it'll never be re-requested.
          </p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="glass-panel rounded-3xl p-8 text-center text-slate-500 text-xs font-bold uppercase tracking-wider">
          No On Hold or Rejected items are waiting to be imported.
        </div>
      ) : (
