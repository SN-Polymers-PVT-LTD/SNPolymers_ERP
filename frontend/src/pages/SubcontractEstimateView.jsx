...
const SubcontractEstimateView = () => {
  ...
  const revisionLog = useMemo(() => estimate?.subcontract_estimate_revision_log || [], [estimate]);
  const latestRevision = useMemo(() => [...revisionLog].sort((a, b) => new Date(b.created_at) - new Date(a.created_at))[0], [revisionLog]);
  ...
  const canReopen = ['Final Approved', 'Rejected by ZO', 'Rejected by HO'].includes(estimate?.estimate_status)
  ...
  const executeWorkflow = async (action, actionRemarks = null) => {
    ...
    try {
      ...
      // Auto-save any row decisions when reviewer is requesting revision or rejecting
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        ...
        {latestRevision?.revision_deadline && (
          <span className="text-xs font-mono text-amber-300/80">
            Deadline: {new Date(latestRevision.revision_deadline).toLocaleString('en-IN')}
          </span>
        )}
      </div>
      <div className="text-sm text-slate-200">
        <span className="font-semibold text-amber-300">Revision Remarks: </span>
        <span>{estimate.zo_remarks || estimate.ho_remarks || 'Please review and revise the estimate lines.'}</span>
      </div>
      <p className="text-xs text-slate-400">
        Review the individual item decisions and comments below. Click "Edit Draft" above to make necessary corrections and resubmit.
      </p>
    </div>}
