...
import {
  getProjectsBeneficiaries, upsertProjectsBeneficiary, getIndianBanks, upsertIndianBank
...
const AddBeneficiaryModal = ({ bankOptions, onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!accountNumberRegex.test(form.beneficiary_ac_no.trim())) return setError('Account number must be 9-18 digits.');
    if (!ifscRegex.test(form.beneficiary_ifsc.trim())) return setError('IFSC must be 11-char in format AAAA0XXXXXX.');
    ...
    setSubmitting(true);
    try {
      await upsertProjectsBeneficiary({
        beneficiary_ac_no: form.beneficiary_ac_no.trim(),
      ...
      queryClient.invalidateQueries({ queryKey: ['projectsBeneficiaries'] });
      onClose();
    } catch (err) {
    ...
    <Modal
      ...
      footer={
        <div className="flex justify-end gap-3 w-full">
          ...
          <Button type="submit" variant="primary" form="fin-beneficiary-form" disabled={submitting} loading={submitting}>
            Add Beneficiary
          </Button>
        </div>
      }
    >
      <form id="fin-beneficiary-form" onSubmit={handleSubmit} className="space-y-4 text-left">
        <Input
          label="Account Number"
          required
          value={form.beneficiary_ac_no}
          onChange={(e) => setField('beneficiary_ac_no', e.target.value.replace(/\D/g, '').slice(0, 18))}
          maxLength={18}
        />
        <Input
          label="IFSC"
          ...
          onChange={(e) => setField('beneficiary_ifsc', e.target.value.toUpperCase())}
...
const AddBankModal = ({ onClose }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      queryClient.invalidateQueries({ queryKey: ['finIndianBanks'] });
      onClose();
    } catch (err) {
  ...
  return (
    <Modal
      ...
      footer={
        <div className="flex justify-end gap-3 w-full">
          <Button variant="secondary" onClick={onClose} disabled={submitting}>Cancel</Button>
          <Button type="submit" variant="primary" form="fin-bank-form" disabled={submitting} loading={submitting}>
            Add Bank
          </Button>
        </div>
      }
    >
      <form id="fin-bank-form" onSubmit={handleSubmit} className="space-y-4 text-left">
...
const BeneficiariesTab = () => {
  ...
  const { data, isLoading, error: queryError } = useQuery({
    queryKey: ['projectsBeneficiaries', { page, search }],
    queryFn: async () => {
      const params = { page, limit: 20 };
      if (search) params.search = search;
      return (await getProjectsBeneficiaries(params)).data;
    }
  });
        ...
        <div className="glass-panel rounded-3xl border border-white/5 overflow-hidden">
          <Table containerClassName="min-w-[700px]">
            ...
            <TableBody>
              {beneficiaries.map((b) => (
                <TableRow key={b.id}>
                  <TableCell className="font-mono">{b.beneficiary_ac_no}</TableCell>
                  <TableCell className="font-mono">{b.beneficiary_ifsc}</TableCell>
...
/**
 * ProjectsBeneficiaryMaster
 * Financial Twin equivalent of Accounts' Beneficiary & Bank Master page.
 * Rows are also auto-populated as JEs fill in beneficiary banking details on
 * requisitions (see ProjectBeneficiarySuggestions / Requisitions.jsx); this
 * page additionally lets Financial Twin users add/correct entries directly
 * and manage the shared indian_bank_master list, mirroring the Accounts flow.
 */
const ProjectsBeneficiaryMaster = () => {
    ...
    <>
      <div className="mb-6 pb-6 border-b border-white/5">
        ...
        <p className="text-xs text-slate-400 font-medium mt-1.5">
          ...
          the same account number and IFSC format enforced on the Accounts Beneficiary Master.
        </p>
      </div>
...
export default ProjectsBeneficiaryMaster;
