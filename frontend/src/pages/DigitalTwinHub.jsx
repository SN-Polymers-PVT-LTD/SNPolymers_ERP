...
import { useDigitalTwinHubUrlState } from '../hooks/useDigitalTwinHubUrlState';
...
const DigitalTwinHub = () => {
  ...
  const uniqueZones = useMemo(() => {
    return [...new Set(projects.map(p => p.zone).filter(Boolean))];
  }, [projects]);
  ...
  }, [projects, searchTerm, statusFilter, zoneFilter]);

  const CARDS_PER_PAGE = 6;
  const totalPages = Math.max(1, Math.ceil(filteredProjects.length / CARDS_PER_PAGE));
  const paginatedProjects = useMemo(() => {
    return filteredProjects.slice((page - 1) * CARDS_PER_PAGE, page * CARDS_PER_PAGE);
  }, [filteredProjects, page, CARDS_PER_PAGE]);

  const hasActiveFilters = Boolean(searchTerm || statusFilter || zoneFilter);
  ...
  return (
    <>
      {/* Header Row */}
      <div className="mb-10 pb-6 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
        <div>
        ...
        {hasActiveFilters && (
          <button
            ...
            className="px-3.5 py-1.5 rounded-xl border border-white/10 hover:border-white/20 bg-white/5 hover:bg-white/10 text-slate-300 text-xs font-bold uppercase tracking-wider transition-all"
          >
            Reset Filters
          </button>
        )}
        ...
        <div className="glass-panel p-16 rounded-3xl text-center flex flex-col items-center justify-center border border-white/5 bg-slate-900/10">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="mt-4 px-4 py-2 rounded-xl bg-white/5 border border-white/10 hover:bg-white/10 text-xs font-bold text-amber-400 uppercase tracking-wider transition-all"
            >
              Clear Filters
            </button>
          )}
