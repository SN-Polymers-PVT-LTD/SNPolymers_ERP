...
const ZonalBalances = () => {
  ...
  const fetchLedger = async (targetPage, zoId = '') => {
    setLoadingLedger(true);
    try {
      const response = await getZonalLedger(targetPage, limit, zoId);
      if (response.data?.success) {
  ...
  // Filter balances list for rendering
  ...
  const filteredBalances = balances.filter(b => {
    ...
    if (selectedZo && b.zo_user_id !== selectedZo) return false;
    const matchesSearch =
  ...
  const renderSortIcon = (key) => {
    ...
    return sortAsc ? <span className="text-amber-400 ml-1 font-bold">▲</span> : <span className="text-amber-400 ml-1 font-bold">▼</span>;
  };
  ...
  return (
    <>
      ...
      <div className="mb-8 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 pb-6 border-b border-white/5">
        ...
        {!isZo && (
          <button
            ...
            className="px-4 py-2.5 rounded-xl text-xs font-bold uppercase bg-white text-black hover:bg-slate-200 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-300 flex items-center gap-2 shadow-lg"
      ...
      {/* Global Filter Bar for Zonal Office selection */}
      {!isZo && (
        <div className="glass-panel p-4 rounded-2xl border border-white/5 mb-8 flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <span className="text-xs font-bold uppercase tracking-wider text-slate-300 shrink-0">Filter Zonal Office:</span>
            <select
              value={selectedZo}
              onChange={(e) => handleZoFilterChange(e.target.value)}
              className="bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-xs text-slate-100 focus:outline-none focus:border-amber-500/50 transition-all font-semibold cursor-pointer"
            >
              <option value="" className="bg-slate-900 text-slate-200">All Active Zonal Offices</option>
              {balances.filter(b => !UNMAPPED_EXCLUDED_MOBILES.includes(b.zo_user_id)).map(b => (
                <option key={b.zo_user_id} value={b.zo_user_id} className="bg-slate-900 text-slate-200">
                  {b.zo_name || b.zo_user_id} ({b.zo_user_id})
                </option>
              ))}
            </select>
          </div>
      ...
      {/* Available Zonal Balances Widget / Grid */}
      <div className="mb-10">
          ...
          <div className="glass-panel rounded-3xl overflow-hidden shadow-xl border border-white/5">
            {filteredBalances.length === 0 ? (
              <div className="p-8 text-center text-slate-500 text-xs font-bold uppercase tracking-wider">
                No matching mapped Zonal Offices found.
              </div>
            ) : (
              <>
                ...
                <div className="px-6 py-4 bg-white/2 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3">
                  <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
                    Showing {Math.min((balancesPage - 1) * balancesLimit + 1, filteredBalances.length)} to {Math.min(balancesPage * balancesLimit, filteredBalances.length)} of {filteredBalances.length} Zonal Offices
                  </span>
                  ...
                  {totalBalancesPages > 1 && (
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="px-3 py-1.5 rounded-lg border border-white/10 text-[10px] font-bold uppercase tracking-wider text-slate-300 hover:bg-white/5 disabled:opacity-30 transition"
                      >
                        Prev
                      </button>
      ...
      <div>
        ...
        <div className="glass-panel rounded-3xl overflow-hidden shadow-2xl border border-white/5">
                  ...
                  ledger.map((log) => {
                    ...
                    return (
                      <tr key={log.ledger_id} className="hover:bg-white/2 transition-colors">
                        <td className="px-6 py-4 text-[10px] text-slate-500 font-normal font-mono">
                        ...
                        <td className={`px-6 py-4 text-right font-extrabold text-sm font-mono ${isCredit ? 'text-emerald-400' : 'text-red-400'}`}>
          ...
          <div className="px-6 py-4 bg-white/2 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3">
            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">
              Page {page} of {totalPages || 1} ({totalCount} total transactions)
            </span>
