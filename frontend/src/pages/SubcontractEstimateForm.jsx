    ...
    <div className="space-y-3">
      {lines.map((line, index) => {
        ...
        return <div key={line.line_id || `new-${index}`} className="grid grid-cols-1 gap-2 rounded-xl border border-white/10 bg-white/[0.02] p-4 md:grid-cols-7">
          ...
          {reconciliationAuthoring && revisionAuthoring && !locked ? <Select value={line.entry_kind || 'ADDITION'} onChange={e => updateKind(index, e.target.value)} options={[{ value: 'ADDITION', label: 'Addition' }, { value: 'ADJUSTMENT', label: 'Adjustment' }]} /> : <Input value={line.entry_kind || 'BASE'} disabled />}
