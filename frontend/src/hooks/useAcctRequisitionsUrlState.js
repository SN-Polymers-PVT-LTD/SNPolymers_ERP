 ...
 *
 * Bi-directionally synchronizes Accounts Requisitions listing state with URL search parameters:
 * - status: 'All' | 'Open' | 'Submitted' | 'Reviewed'
 * - q: sheet number search string (debounced replace)
 * - from: date_from (YYYY-MM-DD)
 * - to: date_to (YYYY-MM-DD)
 * - page: pagination page number
 */
export function useAcctRequisitionsUrlState() {
  const [searchParams, setSearchParams] = useSearchParams();

  // 1. Status filter ('All' | 'Open' | 'Submitted' | 'Reviewed')
  const urlStatus = searchParams.get('status') || searchParams.get('sheet_status') || 'All';
  const validStatuses = ['All', 'Open', 'Submitted', 'Reviewed'];
  const statusFilter = validStatuses.includes(urlStatus) ? urlStatus : 'All';
