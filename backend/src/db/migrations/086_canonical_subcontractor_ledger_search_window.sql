-- Migration 086: Subcontractor Ledger Search Window Order Fix
-- Corrects get_canonical_subcontractor_ledger_entries so that cumulative_paid
-- is calculated across the entire partition before applying the text search filter.
-- This ensures that filtering for a specific requisition or keyword retains the
-- correct cumulative paid disbursements rather than resetting cumulative sums.
...
AS $$
  ...
  filtered AS (
    ...
    WHERE (p_date_from IS NULL OR created_at >= p_date_from)
      ...
      AND (
        p_search IS NULL
        OR subcontractor_name ILIKE '%' || trim(p_search) || '%'
        OR COALESCE(sub_head, material_sub_head) ILIKE '%' || trim(p_search) || '%'
        OR COALESCE(material_details, legacy_material_details) ILIKE '%' || trim(p_search) || '%'
        OR work_order_no ILIKE '%' || trim(p_search) || '%'
        OR requisition_no ILIKE '%' || trim(p_search) || '%'
      )
  ),
  bounds AS (
    SELECT
